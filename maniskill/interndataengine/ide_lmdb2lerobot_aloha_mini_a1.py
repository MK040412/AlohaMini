...
ALOHA_MINI_URDF = (
    REPO_ROOT
    / "InternDataAssets/curobo/src/curobo/content/assets/robot/aloha_mini/aloha_mini.urdf"
)
ALOHA_MINI_JOINT_NAMES = {
    "left": [
        "left_shoulder_pan",
        "left_shoulder_lift",
        "left_elbow_flex",
...
FEATURES = {
    ...
    "states.left_joint.position": {
        "dtype": "float32",
        "shape": (5,),
        "names": ["left_joint_0", "left_joint_1", "left_joint_2", "left_joint_3", "left_joint_4",],
    },
    ...
    "states.right_joint.position": {
        "dtype": "float32",
        "shape": (5,),
        "names": ["right_joint_0", "right_joint_1", "right_joint_2", "right_joint_3", "right_joint_4",],
    },
...
class AlohaMiniDataset(LeRobotDataset):
    def __init__(
        self,
        repo_id: str,
...
def load_lmdb_data(episode_path: Path, sava_path: Path, fps_factor: int, target_fps: int) -> Optional[Dict]:
    ...
    left_armbase_to_robot_pose[:3, 3] = np.array([0.156215, -0.041417, 0.80221])
    right_armbase_to_robot_pose = np.eye(4)
    right_armbase_to_robot_pose[:3, 3] = np.array([-0.15762064, -0.03767275, 0.80290773])
    ...
    tcp2ee_pose[1, 3] = -0.10

    model = pin.buildModelFromUrdf(str(ALOHA_MINI_URDF))
        ...
        with env.begin(write=False) as txn:
            ...
            for compute_qpos_key in compute_qpos_keys:
                ...
                for each_compute_qpos in compute_qpos:
                    q = pin.neutral(model)   # 关节角
                    if "left" in compute_qpos_key:
                        arm_name = "left"
                    elif "right" in compute_qpos_key:
                        arm_name = "right"
                    for joint_name, joint_value in zip(ALOHA_MINI_JOINT_NAMES[arm_name], each_compute_qpos):
                        joint_id = model.getJointId(joint_name)
                        q[model.idx_qs[joint_id]] = joint_value
                    lift_joint_id = model.getJointId("vertical_move")
                    q[model.idx_qs[lift_joint_id]] = ALOHA_MINI_LIFT_HOME
                    pin.forwardKinematics(model, data, q)
                    pin.updateFramePlacements(model, data)
                    fid_a = model.getFrameId(f"{arm_name}_Base")
                    fid_b = model.getFrameId(f"{arm_name}_Fixed_Jaw")
...
def save_as_lerobot_dataset(task: tuple[Path, Path], repo_id, num_threads, debug, origin_fps=30,  target_fps=30, num_demos=None, robot_type="AlohaMini", delete_downsampled_videos=True):
    ...
    dataset = AlohaMiniDataset.create(
        repo_id=f"{repo_id}",
        root=save_path,
        fps=target_fps,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert collected data from AlohaMini to Lerobot format.")
    parser.add_argument(
        "--src_path",
        type=str,
        # required=False,
        default="/fs-computility/efm/shared/datasets/myData-A1/real/raw_data/aloha_mini/",
    ...
    parser.add_argument(
        ...
        type=str,
        # required=False,
        default="/fs-computility/efm/shared/datasets/myData-A1/real/lerobot_v2_1/aloha_mini/",
