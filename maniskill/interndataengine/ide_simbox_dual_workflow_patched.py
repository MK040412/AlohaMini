...
class SimBoxDualWorkFlow(NimbusWorkFlow):
                ...
                if lr_skill_list:
                    ...
                    if start_lr_skill.is_done():
                        if not start_lr_skill.is_success():
                            print(f"[SKILLFAIL] {type(start_lr_skill).__name__} "
                                  f"lr={getattr(start_lr_skill, 'lr_name', getattr(getattr(start_lr_skill, 'controller', None), 'lr_name', '?'))}",
                                  flush=True)
    ...
    def plan_first_skill(self, skills, should_continue):
        for _, robot_skill_list in skills[0].items():
            for lr_skill_list in robot_skill_list[0]:
                ...
                sk = lr_skill_list[0]
                print(f"[PLANFIRST] {type(sk).__name__} "
                      f"lr={getattr(getattr(sk, 'controller', None), 'lr_name', '?')} "
                      f"manip={len(sk.manip_list)} ready={sk.is_ready()}", flush=True)
                ...
                if len(lr_skill_list[0].manip_list) == 0:
                    ...
                    print(f"[PLANFIRST] empty manip -> should_continue={should_continue}", flush=True)
    ...
    def generate_seq(self) -> list:
        ...
        print(f"[LOOPENTER] max={max_episode_length} skills={len(self.skills)} cont={should_continue}", flush=True)
        while not (step_id >= max_episode_length or (not self.skills and not episode_success) or (not should_continue)):
                ...
                for robot_name, skill_sequences in current_skills.items():
                    if skill_sequences and skill_sequences[0]:
                        ...
                        if False in feasible_labels:
                            print(f"[FEASFAIL] {[type(sk[0]).__name__ for sk in skill_sequences[0] if sk[0]]} "
                                  f"fails={[sk[0].controller.num_plan_failed for sk in skill_sequences[0] if sk[0]]}",
                                  flush=True)
                            should_continue = False
            ...
            self.world.step(render=bool(os.environ.get("IDE_VIDEO")))

            step_id += 1
            if step_id % 100 == 0:
                print(f"[LOOPHB] step={step_id} skills={len(self.skills)} "
                      f"succ={episode_success} cont={should_continue}", flush=True)
            if os.environ.get("IDE_VIDEO") and step_id % 5 == 0:
                try:
                    ...
                    for _cn, _cam in self.task.cameras.items():
                        ...
                        if not getattr(self, "_dbgcam_set", False):
                            ...
                            print("[VIDCAM] head camera retargeted", flush=True)
                        _img = _cam.get_observations()["color_image"]
                        self._vid_n = getattr(self, "_vid_n", 0) + 1
                        _iio.imwrite(f"/tmp/ide_frames/v{self._vid_n:05d}.png",
                                     _np.asarray(_img)[..., :3].astype(_np.uint8))
                        break
                except Exception as _e:
                    print(f"[FRAMEDUMP] {_e}", flush=True)
            if self.skills:
