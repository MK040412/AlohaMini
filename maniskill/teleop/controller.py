...
class TeleopController:
    ...
    Action format for aloha_mini_virtual (pd_joint_pos mode):
    ...
    def __init__(self, config: TeleopConfig = None):
        ...
        # Initialize kinematics (AlohaMini URDF geometry)
        self.kinematics = AlohaMiniKinematics()

        # Initialize arm states
        # Note: ArmState uses ee_x, ee_y naming but we interpret as:
        #   ee_x = URDF Y (forward direction, usually negative for this arm)
        #   ee_y = URDF Z (height)
        self.left_arm = ArmState(
            ee_x=self.config.initial_ee_y,  # URDF Y
            ee_y=self.config.initial_ee_z,  # URDF Z
        )
    ...
    def _update_arm_ik(self, arm: ArmState):
        """
        Update arm IK (joints 2, 3, 4) based on current EE position.
        ...
        # Compute wrist flex (joint 4) for pitch compensation
    ...
    def _apply_vr_goal_to_arm(self, arm: ArmState, goal: ControlGoal):
        ...
        # Update IK
        if goal.ee_x is not None or goal.ee_y is not None:
        ...
        if goal.gripper_closed is not None:
            ...
            # Map gripper state to angle for AlohaMini
            # AlohaMini gripper: 0° = closed, positive = open
            arm.joint6_deg = 0.0 if goal.gripper_closed else 45.0
    ...
    def reset_arms(self):
        ...
        self._update_arm_ik(self.left_arm)
        self._update_arm_ik(self.right_arm)
        print("Arms reset to initial position")
    ...
    def set_lift_position(self, position: float):
        ...
        Args:
            position: Lift position in meters (0 to 0.6)
        """
        self.lift_position = max(0.0, min(0.6, position))
    ...
    def compute_action(self) -> np.ndarray:
        ...
        # Left arm joints (indices 4-9)
        left_joints_rad = self.left_arm.get_joint_positions_rad()
        action[self.LEFT_ARM_START:self.LEFT_ARM_END] = left_joints_rad

        # Right arm joints (indices 10-15)
        right_joints_rad = self.right_arm.get_joint_positions_rad()
        action[self.RIGHT_ARM_START:self.RIGHT_ARM_END] = right_joints_rad
