...
LEFT_ARM_MAPPING = {
    ...
    pygame.K_w: ('ee_x', -1),        # EE forward (x decrease in XLeRobot convention)
    pygame.K_s: ('ee_x', +1),        # EE backward (x increase)
    pygame.K_e: ('ee_y', -1),        # EE down (y decrease)
    pygame.K_d: ('ee_y', +1),        # EE up (y increase)
...
RIGHT_ARM_MAPPING = {
    ...
    pygame.K_o: ('ee_y', -1),        # EE down
    pygame.K_l: ('ee_y', +1),        # EE up
...
class KeyboardController:
    ...
    def process_keys(self, keys_pressed: pygame.key.ScancodeWrapper) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with control deltas for left and right arms
        """
        result = {
    ...
    def _apply_action(self, delta: Dict[str, float], action_type: str, direction: int):
        """
        Apply keyboard action to delta dictionary.
